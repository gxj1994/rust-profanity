...
def pack_words_to_ulong(words):
    """将单词列表打包为ulong数组，每个ulong存储8个字符"""
    # 计算总字符数
    total_chars = sum(len(w) for w in words)
    # 计算需要的ulong数量 (每个ulong 8字节 = 8字符)
    num_ulongs = (total_chars + 7) // 8
    ...
    for word in words:
        for char in word.encode('utf-8'):
            # 将字符放入当前ulong的适当位置 (大端序)
            shift = (7 - (char_count % 8)) * 8
            current_ulong |= (char << shift)
            char_count += 1
            
            if char_count % 8 == 0:
                ulongs.append(current_ulong)
                current_ulong = 0
    ...
    return ulongs, total_chars


def generate_offsets(words):
    """生成每个单词的偏移量和长度数组"""
    offsets = []
    lengths = []
    current_offset = 0
...
def generate_opencl_wordlist():
    """生成OpenCL词表文件"""
    
    # 生成偏移量和长度数组
    offsets, lengths = generate_offsets(BIP39_WORDS)
    
    # 打包单词数据
    ulongs, total_chars = pack_words_to_ulong(BIP39_WORDS)
    
    print(f"总字符数: {total_chars}")
    print(f"ulong数组大小: {len(ulongs)} ({len(ulongs) * 8} bytes)")
    print(f"偏移量数组大小: {len(offsets)} ({len(offsets) * 2} bytes)")
    print(f"长度数组大小: {len(lengths)} ({len(lengths)} bytes)")
    print(f"总大小: {len(ulongs) * 8 + len(offsets) * 2 + len(lengths)} bytes")
    
    # 生成OpenCL代码
    lines = []
    lines.append("// BIP39 英文词表 (紧凑存储)")
    lines.append("// 自动生成，请勿手动修改")
    lines.append("")
    lines.append(f"#define BIP39_WORD_COUNT {len(BIP39_WORDS)}")
    lines.append(f"#define BIP39_TOTAL_CHARS {total_chars}")
    lines.append(f"#define BIP39_ULONG_COUNT {len(ulongs)}")
    lines.append("")
    
    # 单词长度数组
    lines.append("// 每个单词的长度 (2048 bytes)")
    lines.append("__constant uchar WORD_LENGTHS[BIP39_WORD_COUNT] = {")
    length_lines = []
    for i in range(0, len(lengths), 16):
        chunk = lengths[i:i+16]
        length_lines.append("    " + ", ".join(f"{l:2d}" for l in chunk))
    lines.append(",\n".join(length_lines))
    lines.append("};")
    lines.append("")
    
    # 单词偏移量数组
    lines.append("// 每个单词在字符数据中的偏移量 (4096 bytes)")
    lines.append("__constant ushort WORD_OFFSETS[BIP39_WORD_COUNT] = {")
    offset_lines = []
    for i in range(0, len(offsets), 8):
        chunk = offsets[i:i+8]
        offset_lines.append("    " + ", ".join(f"{o:5d}" for o in chunk))
    lines.append(",\n".join(offset_lines))
    lines.append("};")
    lines.append("")
    
    # 单词数据数组 (打包为ulong)
    lines.append(f"// 单词字符数据 ({len(ulongs) * 8} bytes)")
    lines.append(f"__constant ulong WORD_DATA[{len(ulongs)}] = {{")
    data_lines = []
    for i in range(0, len(ulongs), 4):
        chunk = ulongs[i:i+4]
        data_lines.append("    " + ", ".join(f"0x{v:016X}ULL" for v in chunk))
    lines.append(",\n".join(data_lines))
    lines.append("};")
    lines.append("")
    ...
    # 添加辅助函数
    lines.append("// 获取单词长度")
    lines.append("uchar get_word_length(ushort idx) {")
    lines.append("    return WORD_LENGTHS[idx];")
    lines.append("}")
    lines.append("")
    
    lines.append("// 将单词复制到目标缓冲区")
    lines.append("// 返回复制的字节数")
    lines.append("uchar copy_word(ushort idx, uchar* dest, uchar max_len) {")
    lines.append("    ushort offset = WORD_OFFSETS[idx];")
    lines.append("    uchar len = WORD_LENGTHS[idx];")
    lines.append("    if (len > max_len) len = max_len;")
    lines.append("    ")
    lines.append("    for (uchar i = 0; i < len; i++) {")
    lines.append("        ushort char_idx = offset + i;")
    lines.append("        ushort ulong_idx = char_idx >> 3;  // / 8")
    lines.append("        uchar bit_offset = (7 - (char_idx & 7)) << 3;  // % 8, then * 8")
    lines.append("        dest[i] = (uchar)(WORD_DATA[ulong_idx] >> bit_offset);")
    lines.append("    }")
